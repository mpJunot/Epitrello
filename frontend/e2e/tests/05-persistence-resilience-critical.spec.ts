...
test.describe('Data Persistence - Critical Paths Only', () => {
  ...
  test('List order persists after refresh', async ({ page }) => {
    ...
    const listIds1 = await listElements.evaluateAll(els => els.map((el: HTMLElement | SVGElement) => (el as HTMLElement).textContent?.substring(0, 20)));
    ...
    const listIds2 = await page.locator('[class*="list"]').evaluateAll(els => els.map((el: HTMLElement | SVGElement) => (el as HTMLElement).textContent?.substring(0, 20)));
