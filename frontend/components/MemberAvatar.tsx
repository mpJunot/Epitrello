...
export default function MemberAvatar({ user, size = 8 }: { user: { id: string; name?: string; avatar?: string; email?: string }; size?: number }) {
  ...
  const dim = `${size}rem`;
  return (
    <div title={user.name || user.email} className="rounded-full bg-trello-border flex items-center justify-center text-xs text-trello-secondary" style={{ width: dim, height: dim }}>
