    ...
    <div>
      ...
      <div className="ml-7 mb-4">
        <div className="flex gap-2 items-start">
          <div className="flex-shrink-0 w-8 h-8 rounded-full bg-trello-blue text-white flex items-center justify-center text-xs font-medium">
            {currentUser.avatar ? (
          ...
          <div className="flex-1">
            ...
            {newComment.trim() && (
              <div className="flex gap-2 mt-2">
                ...
                <Button onClick={() => onChangeNewComment("")} variant="secondary" size="sm">
                  Cancel
                </Button>
              </div>
            )}
            ...
            <div key={comment.id} className="flex gap-2 items-start group">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-sm font-medium text-trello">{comment.author.name || "User"}</span>
                  <span className="text-xs text-trello-text-secondary">{formatCommentDate(comment.createdAt)}</span>
                </div>
                ...
                {editingCommentId === comment.id ? (
                  <div>
                    ...
                    />
                    <div className="flex gap-2 mt-2">
                      <Button onClick={() => onSaveEditComment(comment.id)} size="sm">
                        Save
                      </Button>
                      <Button onClick={onCancelEditComment} variant="secondary" size="sm">
                        Cancel
                      </Button>
                    </div>
                  </div>
                ) : (
                  <div className="bg-trello-hover px-3 py-2 rounded-md text-sm text-trello whitespace-pre-wrap break-words">
                    {comment.text}
                  </div>
                )}
                ...
                {editingCommentId !== comment.id && comment.author.id === currentUser.id && (
                  <div className="flex gap-2 mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button onClick={() => onStartEditComment(comment)} variant="ghost" size="sm" className="text-xs h-auto p-0">
                      <Edit2 className="w-3 h-3 mr-1" />
                      Edit
                    </Button>
                    <span className="text-xs text-trello-border">•</span>
                    <Button onClick={() => onDeleteComment(comment.id)} variant="ghost" size="sm" className="text-xs text-red-600 hover:text-red-700 h-auto p-0">
