...
export default function AddToCardMenu({
  ...
  return (
    <div>
      <h3 className="text-xs font-semibold text-trello-text-secondary uppercase tracking-wide mb-2">Add to card</h3>
      <div className="space-y-2">
        <div className="relative" ref={(el) => registerMenuRef('members', el)}>
          <Button onClick={() => toggleMenu('members')} variant="ghost" className="w-full justify-start">
            ...
            <div className="absolute top-full left-0 mt-1 w-64 bg-trello-card-bg rounded-lg shadow-xl border border-trello-border p-3 z-10 animate-fade-in">
              ...
              <div className="space-y-1 max-h-64 overflow-y-auto custom-scrollbar">
                {availableMembers.map((member) => {
                  ...
                  return (
                    <Button key={member.id} onClick={() => toggleMember(member)} variant="ghost" className={`w-full justify-start ${isAssigned ? 'bg-trello-blue-light hover:bg-trello-blue-light' : ''}`}>
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium text-white ${isAssigned ? 'bg-trello-blue' : 'bg-trello-text-secondary'}`}>
                        {member.avatar ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium text-trello truncate">{member.name}</div>
                        <div className="text-xs text-trello-text-secondary truncate">{member.email}</div>
                      </div>
                      {isAssigned && (
                        <Check className="w-5 h-5 text-trello-blue flex-shrink-0" />
                      )}
                    </Button>
                  );
                })}
        ...
        <div className="relative" ref={(el) => registerMenuRef('labels', el)}>
          <Button onClick={() => toggleMenu('labels')} variant="ghost" className="w-full justify-start">
            ...
            <div className="absolute top-full left-0 mt-1 w-64 bg-trello-card-bg rounded-lg shadow-xl border border-trello-border p-3 z-10 animate-fade-in">
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto custom-scrollbar">
                {availableLabels.map((label) => {
                  ...
                  return (
                    <Button key={label.id} onClick={() => toggleLabel(label)} variant="ghost" className={`w-full justify-start ${label.color} text-white hover:opacity-90 ${isAssigned ? 'ring-2 ring-trello-blue ring-offset-2' : ''}`}>
        ...
        <div className="relative" ref={(el) => registerMenuRef('checklist', el)}>
          <Button onClick={() => toggleMenu('checklist')} variant="ghost" className="w-full justify-start">
        ...
        <div className="relative" ref={(el) => registerMenuRef('dates', el)}>
          <Button onClick={() => toggleMenu('dates')} variant="ghost" className="w-full justify-start">
          ...
          {openMenu === 'dates' && (
            <div className="absolute top-full left-0 mt-1 w-64 bg-trello-card-bg rounded-lg shadow-xl border border-trello-border p-3 z-10 animate-fade-in">
              ...
              <Button onClick={saveDueDate} disabled={!selectedDate} className="w-full" size="sm">
                Save
              </Button>
              {dueDate && (
                <Button onClick={removeDueDate} variant="destructive" className="w-full mt-2" size="sm">
                  Remove
                </Button>
              )}
            </div>
