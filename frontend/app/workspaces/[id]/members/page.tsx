    ...
    <div className="min-h-screen bg-trello-hover p-6">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-semibold text-trello">Workspace members</h1>
          <div>
            <Button onClick={() => setShowInviteDialog(true)}>Invite</Button>
          </div>
        </div>

        <div className="bg-trello-card-bg rounded shadow p-4">
          {members.length === 0 && <div className="text-trello-text-secondary">No members</div>}
          <ul className="divide-y divide-trello-border">
            {members.map(m => (
              <li key={m.id} className="py-3 flex items-center justify-between">
                <div>
                  <div className="font-medium text-trello">{m.name}</div>
                  <div className="text-xs text-trello-text-secondary">{m.email}</div>
                </div>
                <div>
